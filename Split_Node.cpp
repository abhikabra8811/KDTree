...
template<typename T>
Split_Node<T>::Split_Node(vector<Point<T>> & list_points, int split_dim) {
	...
	if (list_points.size() > 1) {		
		...
		auto left = vector<Point<T>>(begin(list_points), begin(list_points) + list_points.size() / 2);
		auto right = vector<Point<T>>(begin(list_points) + list_points.size() / 2, end(list_points));
		
		//if number of points on left are greater than 1 then create a split
		if (left.size() > 1) {
			left_ = make_shared<Split_Node<T>>(vector<Point<T>>(begin(list_points), begin(list_points) + list_points.size() / 2), 0);
		}
		else {//only 1 point create a leaf node to store the point
			left_ = make_shared<Point_Node<T>>(left[0]);
		}

		//if number of points on right are greater than 1 then create a split
		if (right.size() > 1){
			right_ = make_shared<Split_Node<T>>(vector<Point<T>>(begin(list_points) + list_points.size() / 2, end(list_points)), 0);
		}
		else {
			right_ = make_shared<Point_Node<T>>(right[0]);
		}
...
bool Split_Node<T>::findPoint(const Point<T>& input_pt) const{
	if (input_pt[split_dimension_] < split_value_){
		return (!this->left_->findPoint(input_pt));
	}
	else{
		return (!this->right_->findPoint(input_pt));
	}
}

template<typename T>
void Split_Node<T>::nearestNeighbor(const Point<T>& input_pt, int & index, T & best_dist) const{
